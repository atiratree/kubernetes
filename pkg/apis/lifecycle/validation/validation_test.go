...
func TestValidateEvictionRequestStatusUpdate(t *testing.T) {
	...
	successCases := map[string]struct {
		...
		"Evicted condition can be reverted to false": {
			oldInput: mkValidEvictionRequestStatus(1, addERCondition(clock, lifecycle.EvictionConditionTargetEvicted, true)),
			input:    mkValidEvictionRequestStatus(1, addERCondition(clock, lifecycle.EvictionConditionTargetEvicted, false)),
		},
		// this is different from Eviction
		"Failed condition can be reverted to false": {
			oldInput: mkValidEvictionRequestStatus(1, addERCondition(clock, lifecycle.EvictionConditionFailed, true)),
			input:    mkValidEvictionRequestStatus(1, addERCondition(clock, lifecycle.EvictionConditionFailed, false)),
		},
		// observedGeneration
		...
		"update generation": {
			...
			input:    mkValidEvictionRequestStatus(0, setERObservedGeneration(new(int64(5)))),
		},
	}
	for name, tc := range successCases {
		t.Run(name, func(t *testing.T) {
	...
	failureCases := map[string]struct {
		...
		// observedGeneration
		"clear generation": {
			oldInput: mkValidEvictionRequestStatus(0, setERObservedGeneration(new(int64(1)))),
		...
		"decrease generation": {
			oldInput: mkValidEvictionRequestStatus(0, setERObservedGeneration(new(int64(2)))),
			input:    mkValidEvictionRequestStatus(0, setERObservedGeneration(new(int64(1)))),
			errors: []*field.Error{
...
func TestValidateEvictionStatusUpdate(t *testing.T) {
	...
	successCases := map[string]struct {
		...
		"finalize eviction request active - large number of responders": {
			...
			oldInput: mkValidEvictionStatus(16,
				setStateFor(lifecycle.ResponderStateActive, 15),
	...
	failureCases := map[string]struct {
		...
		"change and remove a requester": {
			oldInput: mkValidEvictionStatus(0, clearRequesters(), addRequesters(lifecycle.RequesterIntentEviction, "foo.example.com/baz")),
			input:    mkValidEvictionStatus(0, clearRequesters(), addRequesters(lifecycle.RequesterIntentEviction, "bar.example.com/bay")),
			errors: field.ErrorList{
				field.Invalid(field.NewPath("status", "requesters"), "", "requesters cannot be removed"),
			},
		},
		"requester with non conformant name cannot be added": {
			oldInput: mkValidEvictionStatus(0),
			input:    mkValidEvictionStatus(0, addRequesters(lifecycle.RequesterIntentEviction, "foo.example.com")),
			errors: []*field.Error{
...
func setERName(name string) func(obj *lifecycle.EvictionRequest) {
...
func addTargetResponders(responders ...string) func(obj *lifecycle.EvictionStatus) {
	return func(obj *lifecycle.EvictionStatus) {
		for _, responderName := range responders {
			obj.TargetResponders = append(obj.TargetResponders, lifecycle.TargetResponder{Name: responderName, State: lifecycle.ResponderStateInactive})
		}
	}
}
