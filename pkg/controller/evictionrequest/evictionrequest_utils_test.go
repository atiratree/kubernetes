...
func TestFindRelevantEviction(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			evictions: []*lifecyclev1alpha1.Eviction{
				mkValidEviction("pod-1-foo", "foo", "uid-1",
					addConditionTrue(clockOldest, lifecyclev1alpha1.EvictionConditionFailed, lifecyclev1alpha1.EvictionConditionReasonNoFurtherResponder)),
				mkValidEviction("pod-2-foo", "foo", "uid-1",
					addConditionTrue(clock, lifecyclev1alpha1.EvictionConditionFailed, lifecyclev1alpha1.EvictionConditionReasonNoFurtherResponder)),
			},
			expectedEviction: mkValidEviction("pod-2-foo", "foo", "uid-1",
				addConditionTrue(clock, lifecyclev1alpha1.EvictionConditionFailed, lifecyclev1alpha1.EvictionConditionReasonNoFurtherResponder)),
			expectedShouldCreate: true,
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.expectedEviction, relevantEviction); len(diff) > 0 {
				t.Fatalf("unexpected relevant eviction (-want +got):\n%s", diff)
			}
...
func TestConvertToEvictionRequestConditions(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			evictionConditions: []metav1.Condition{
				{Type: "ResponderCondition", Status: metav1.ConditionTrue,
					Reason: "foo", Message: "msg2"},
				{Type: string(lifecyclev1alpha1.EvictionConditionTargetEvicted), Status: metav1.ConditionTrue,
					Reason: string(lifecyclev1alpha1.EvictionConditionReasonPodDeleted), Message: "msg", LastTransitionTime: metav1.Time{Time: clock.Now()}},
				{Type: string(lifecyclev1alpha1.EvictionConditionFailed), Status: metav1.ConditionFalse,
					Reason: string(lifecyclev1alpha1.EvictionConditionReasonSucceeded), Message: "msg2", LastTransitionTime: metav1.Time{Time: clock.Now()}},
			},
			conditionType: lifecyclev1alpha1.EvictionConditionTargetEvicted,
			expectedApplyConfiguration: metav1ac.Condition().
				WithType(string(lifecyclev1alpha1.EvictionConditionTargetEvicted)).
		...
		{
			name: "find correct condition with no change",
			existingConditions: []metav1.Condition{
				{Type: string(lifecyclev1alpha1.EvictionConditionTargetEvicted), Status: metav1.ConditionTrue,
					Reason: string(lifecyclev1alpha1.EvictionConditionReasonPodDeleted), Message: "msg", LastTransitionTime: metav1.Time{Time: clockOld.Now()}},
			},
			evictionConditions: []metav1.Condition{
				{Type: "ResponderCondition", Status: metav1.ConditionTrue,
					Reason: "foo", Message: "msg2"},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, changed := convertToEvictionRequestConditions(clock.Now(), evictionRequest, eviction, tc.conditionType)
...
func TestEvictionRequestAsOwnerReference(t *testing.T) {
	...
	if diff := cmp.Diff(expectedOwnerRef, gotOwnerRef); len(diff) > 0 {
		t.Errorf(" unexpected eviction request ownerRef (-want +got):\n%s", diff)
	}
}

func TestComputeEvictionLabels(t *testing.T) {
	tests := []struct {
		name                  string
		eviction              *lifecyclev1alpha1.Eviction
		target                targetInfo
		targetEvictionRequest []*lifecyclev1alpha1.EvictionRequest
		expectedLabels        map[string]string
	}{
		{
			name: "computes labels",
			eviction: mkValidEviction("pod-1-foo", "foo", "uid-1", setLabels(map[string]string{
				"foo": "bar", // not used - computeEvictionLabels computes just the SSA update
			})),
			target: newTargetInfoForEviction(mkValidPodTarget("my-pod", "uid-1"),
				mkValidPod("my-pod", "uid-1", setEvictionResponders(
					v1.EvictionResponder{Name: "responder-a.example.com/a", Priority: new(int32(1000))},
					v1.EvictionResponder{Name: "responder-b.example.com/b", Priority: new(int32(15000))},
				)),
			),
			targetEvictionRequest: []*lifecyclev1alpha1.EvictionRequest{
				mkValidEvictionRequest("requester-1", "pod-1", setERRequester("foo.example.com/1")),
				mkValidEvictionRequest("requester-2", "pod-2", setERRequester("responder-a.example.com/a")),
			},
			expectedLabels: map[string]string{
				"foo.example.com/1":                                   string(lifecyclev1alpha1.EvictionParticipantRoleRequester),
				"responder-a.example.com/a":                           string(lifecyclev1alpha1.EvictionParticipantRoleRequesterResponder),
				"responder-b.example.com/b":                           string(lifecyclev1alpha1.EvictionParticipantRoleResponder),
				lifecyclev1alpha1.EvictionResponderImperativeEviction: string(lifecyclev1alpha1.EvictionParticipantRoleResponder),
			},
		},
		{
			name: "target gone persists labels",
			eviction: mkValidEviction("pod-1-foo", "foo", "uid-1", setLabels(map[string]string{
				"foo":                       "bar", // not used - computeEvictionLabels computes just the SSA update
				"foo.example.com/1":         string(lifecyclev1alpha1.EvictionParticipantRoleRequester),
				"responder-a.example.com/a": string(lifecyclev1alpha1.EvictionParticipantRoleRequesterResponder),
		...
		{
			name: "target gone persists labels, but requesters rotated",
			eviction: mkValidEviction("pod-1-foo", "foo", "uid-1", setLabels(map[string]string{
				"foo":                       "bar", // not used - computeEvictionLabels computes just the SSA update
				"foo.example.com/1":         string(lifecyclev1alpha1.EvictionParticipantRoleRequester),
				"responder-a.example.com/a": string(lifecyclev1alpha1.EvictionParticipantRoleRequesterResponder),
			...
			target: newTargetInfoForEviction(mkValidPodTarget("my-pod", "uid-1"), nil),
			targetEvictionRequest: []*lifecyclev1alpha1.EvictionRequest{
				mkValidEvictionRequest("requester-3", "pod-3", setERRequester("bar.example.com/1")),
				mkValidEvictionRequest("requester-4", "pod-4", setERRequester("responder-b.example.com/b")),
			},
			expectedLabels: map[string]string{
				"bar.example.com/1":                                   string(lifecyclev1alpha1.EvictionParticipantRoleRequester),
				"responder-a.example.com/a":                           string(lifecyclev1alpha1.EvictionParticipantRoleResponder),
				"responder-b.example.com/b":                           string(lifecyclev1alpha1.EvictionParticipantRoleRequesterResponder),
				lifecyclev1alpha1.EvictionResponderImperativeEviction: string(lifecyclev1alpha1.EvictionParticipantRoleResponder),
			},
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := computeEvictionLabels(tc.eviction, tc.target, tc.targetEvictionRequest)
			if diff := cmp.Diff(tc.expectedLabels, got); len(diff) > 0 {
				t.Errorf("unexpected eviction labels (-want +got):\n%s", diff)
			}
		})
	}
}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.expected, result); len(diff) > 0 {
				t.Errorf("unexpected sortTargetResponders (-want +got):\n%s", diff)
			}
		})
	}
}
...
func setPodDeletionPhase(phase v1.PodPhase) func(obj *v1.Pod) {
	return func(obj *v1.Pod) {
		obj.Status.Phase = phase
	}
}
func setPodContainerStatuses(statuses ...v1.ContainerStatus) func(obj *v1.Pod) {
	return func(obj *v1.Pod) {
		obj.Status.ContainerStatuses = statuses
	}
}
...
func setERCreationTimestamp(time metav1.Time) func(obj *lifecyclev1alpha1.EvictionRequest) {
	return func(obj *lifecyclev1alpha1.EvictionRequest) {
