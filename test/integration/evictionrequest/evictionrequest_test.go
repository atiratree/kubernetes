...
func TestEvictionRequestValidation(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedFailedCondition: metav1.Condition{
				...
				Status:  metav1.ConditionTrue,
				Reason:  string(lifecyclev1alpha1.EvictionConditionReasonEvictionInvalid),
				Message: "references a SchedulingGroup",
			},
			expectedTargetEvictedCondition: metav1.Condition{
				Type:   string(lifecyclev1alpha1.EvictionConditionTargetEvicted),
				Status: metav1.ConditionFalse,
	...
	for i, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			expectConditions := matchExpectedConditions(tc.expectedFailedCondition, tc.expectedTargetEvictedCondition)
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedTargetEvictedCondition: metav1.Condition{
				Type:    string(lifecyclev1alpha1.EvictionConditionTargetEvicted),
				Status:  metav1.ConditionTrue,
				Reason:  string(lifecyclev1alpha1.EvictionConditionReasonPodTerminal),
				Message: "terminal",
			},
			expectedFailedCondition: metav1.Condition{
				Type:   string(lifecyclev1alpha1.EvictionConditionFailed),
		...
		},
		{
			...
			expectedTargetEvictedCondition: metav1.Condition{
				Type:    string(lifecyclev1alpha1.EvictionConditionTargetEvicted),
				Status:  metav1.ConditionTrue,
				Reason:  string(lifecyclev1alpha1.EvictionConditionReasonPodDeleted),
				Message: "deleted",
			},
			expectedFailedCondition: metav1.Condition{
				Type:   string(lifecyclev1alpha1.EvictionConditionFailed),
	...
	for i, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case tc.cancelEviction:
				...
				expectConditions := matchExpectedConditions(metav1.Condition{
					Type:    string(lifecyclev1alpha1.EvictionConditionTargetEvicted),
					Status:  metav1.ConditionFalse,
					Reason:  string(lifecyclev1alpha1.EvictionConditionReasonEvictionFailed),
					Message: "",
					...
					Type:    string(lifecyclev1alpha1.EvictionConditionFailed),
					Status:  metav1.ConditionTrue,
					Reason:  string(lifecyclev1alpha1.EvictionConditionReasonCanceledDueToNoRequesters),
					Message: "No active requesters",
				})
				tCtx.Eventually(getEviction(eviction)).WithTimeout(controllerResponseTimeout).WithPolling(pollInterval).Should(
			...
			case tc.completeWithoutTermination:
				...
				expectConditions := matchExpectedConditions(metav1.Condition{
					...
					Message: "completed without evicting",
				})
				tCtx.Eventually(getEviction(eviction)).WithTimeout(controllerResponseTimeout).WithPolling(pollInterval).Should(
					gomega.And(expectConditions, expectEvictionGeneration))
...
func matchExpectedConditions(failedCondition, targetEvictedCondition metav1.Condition,
) types.GomegaMatcher {
	return gomega.HaveField("Status.Conditions", gomega.And(gomega.ContainElement(
		gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
			"Type":    gomega.Equal(targetEvictedCondition.Type),
			"Status":  gomega.Equal(targetEvictedCondition.Status),
			"Reason":  gomega.Equal(targetEvictedCondition.Reason),
			"Message": gomega.ContainSubstring(targetEvictedCondition.Message),
		}),
	), gomega.ContainElement(
		gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
			"Type":    gomega.Equal(failedCondition.Type),
			"Status":  gomega.Equal(failedCondition.Status),
			"Reason":  gomega.Equal(failedCondition.Reason),
			"Message": gomega.ContainSubstring(failedCondition.Message),
		}),
	)))
}
